...
@WebServlet(urlPatterns = {"/api/user/huyChuyenXe"})
public class API_HuyChuyenXe extends HttpServlet {
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            String message = chuyenXeService.doiTrangThai(id.trim(), "Đã huỷ");
            resp.setStatus(HttpServletResponse.SC_OK);
            Map<String, String> response = new HashMap<>();
            response.put("status", "success");
            response.put("message", message);
            if (!ghiChu.isEmpty()) 
                response.put("ghiChu", ghiChu);
            mapper.writeValue(resp.getWriter(), response);
        } catch (Exception e) {
            resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            Map<String, String> error = new HashMap<>();
            error.put("status", "error");
            error.put("message", "Không thể hủy chuyến xe: " + e.getMessage());
